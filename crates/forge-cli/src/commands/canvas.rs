...
//! server on its landing page (greenfield blank-canvas mode is a Phase 2
//! placeholder).
//!
//! ## Input
...
//! - `forge-canvas::clone` — site cloning via `clone_url`
...
/// starts on its landing page as a Phase 1 placeholder for greenfield mode.
pub async fn canvas_command(url: Option<String>, port: u16) -> anyhow::Result<()> {
    ...
    let browser_url = if let Some(raw_url) = url {
        ...
        let cloned = yantra_canvas::clone_url(&target_url).await?;
        let parsed_html = scraper::Html::parse_document(&cloned.html);
        ...
        println!("Cloned {} into the canvas.", cloned.base_url);
